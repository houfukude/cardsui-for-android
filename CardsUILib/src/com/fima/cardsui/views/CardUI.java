...
public class CardUI extends FrameLayout {
    ...
    public void restoreCard(Card card){
        int postion = getCardStackPosition(card);
        addCardToStack(card,postion,true);
    }

    public  void addCardToStack(Card card,int postion){
        addCardToStack(card,postion,false);
    }

    public void addCardToStack(Card card, int postion, boolean refresh) {
        if (mStacks.isEmpty()) {
            addCard(card, refresh);
            return;
        }
        CardStack cardStack = (CardStack) mStacks.get(postion);
        cardStack.add(card);
        mStacks.set(postion, cardStack);
        if (refresh)
            refresh();
    }

    public int getCardStackPosition(Card card){
        if (!mStacks.isEmpty()) {
            for(int i=0;i<getLastCardStackPosition();i++){
                CardStack cardStack = (CardStack) mStacks.get(i);
                for (Card tmpCard : cardStack.getCards())
                if(card.hashCode() ==  tmpCard.hashCode()){
                    return i ;
                }
            }
        }
	...
	public void addCardToLastStack(Card card, boolean refresh) {
		...
		int lastItemPos = getLastCardStackPosition();
